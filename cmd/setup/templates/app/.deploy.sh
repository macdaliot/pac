...
aws s3 ls | grep "$1.[[.projectName]].pac.pyramidchallenges.com" >> /dev/null && echo "Bucket exists" || sleep 150

# Sync the files in the newly build `dist` folder with the environment's S3 bucket
aws s3 sync dist s3://$1.[[.projectName]].pac.pyramidchallenges.com --acl public-read
