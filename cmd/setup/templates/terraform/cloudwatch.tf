...
resource "aws_cloudwatch_log_group" "testdns_log_group" {
...
resource "aws_cloudwatch_log_stream" "testdns_log_stream" {
  name           = "${var.project_name}-log-stream"
  log_group_name = "${aws_cloudwatch_log_group.testdns_log_group.name}"
}
