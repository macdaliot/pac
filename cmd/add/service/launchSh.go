...
if [ ! -z "$AWS_ACCESS_KEY_ID" ] || [ ! -z "$AWS_SECRET_ACCESS_KEY" ]; then
  ...
  SERVICE_CONTAINER_PID=$(docker ps -aqf Name=pac-{{.serviceName}}-service)
  ...
  docker run --name pac-{{.serviceName}}-service -p $PORT:3000 --link pac-db-local -e AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY -d pac-{{.serviceName}}-service
